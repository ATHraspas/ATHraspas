...
export function ModulePage({ module: currentModule }: { module: ModuleContent }) {
  ...
  const glossaryMap = Object.fromEntries(currentModule.glossary.map((t)=>[t.term,t]));
  ...
  return <div className="grid xl:grid-cols-[minmax(0,980px)_330px] gap-6">
    <div className="space-y-4">
      <section className={`${block} bg-yellow`}><p className="section-label text-[11px] uppercase">{currentModule.category}</p><h2 className="text-2xl font-black mt-2">{currentModule.title}</h2><p className="font-bold mt-2">{currentModule.hero}</p><p className="mt-2 text-sm leading-relaxed">{currentModule.description}</p></section>
      ...
      <section className="grid lg:grid-cols-2 gap-4">{[['Why this matters',[currentModule.why]],['Advertising use cases',currentModule.useCases],['Real work scenarios',currentModule.scenarios],['Tools needed',currentModule.tools],['Production tactics',currentModule.tips]].map(([t,list])=><div key={String(t)} className={block}><h3 className="font-black text-xl">{t}</h3><ul className="mt-2 list-disc ml-5 text-sm leading-relaxed">{(list as string[]).map(i=>{const t=glossaryMap[i]; return <li key={i}>{t ? <span className="relative group border-b border-dashed border-black cursor-help">{i}<span className="hidden group-hover:block absolute z-20 left-0 top-full mt-1 w-64 bg-white border-[3px] border-black p-2 text-xs"><b>{t.term}</b><p>{t.tr}</p><p>{t.why}</p></span></span> : i}</li>;})}</ul></div>)}</section>

      <section className={block}><h3 className="font-black text-xl">Mini Glossary</h3>
      ...
      <section className={block}><h3 className="font-black text-xl">Common mistakes</h3><div className="space-y-2 mt-2">{currentModule.mistakes.map((m)=><article key={m.problem} className="border-[3px] border-black p-2"><p><b>Sorun:</b> {m.problem}</p><p><b>Sebep:</b> {m.reason}</p><p><b>Çözüm:</b> {m.solution}</p></article>)}</div></section>
      <section className={block}><h3 className="font-black text-xl">Resources</h3><div className="grid md:grid-cols-2 gap-2 mt-2">{currentModule.resources.map((r)=><article key={r.title} className="border-[3px] border-black p-2 bg-[#EFE8D7]"><p className="font-black">{r.title}</p><p className="text-sm">{r.platform} · {r.language} · {r.duration} · {r.level}</p><p className="text-sm">{r.learn}</p><p className="text-sm">Mini pratik: {r.practice}</p></article>)}</div></section>

      <section className={block}><h3 className="font-black text-xl">Assignments ({completed}/{currentModule.assignments.length})</h3><div className="space-y-2 mt-2">{currentModule.assignments.map((a)=>{
        const key=`${currentModule.id}:${a.id}`;
        return <details key={a.id} className="border-[3px] border-black bg-[#EFE8D7]"><summary className="p-2 grid grid-cols-[1fr_auto] items-center gap-2"><div><p className="font-black">{a.title}</p><p className="text-sm"><span className="inline-block border-[3px] border-black px-1 bg-white mr-2">{a.level}</span>{a.shortGoal}</p></div><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={!!done[key]} onChange={()=>toggle(a.id)} /> Tamamlandı</label></summary><div className="p-3 border-t-[3px] border-black text-sm leading-relaxed space-y-1"><p><b>Goal:</b> {a.goal}</p><p><b>Advertising use case:</b> {a.adMatch}</p><p><b>Required programs:</b> {a.programs.join(', ')}</p><p><b>Scene:</b> {a.scene}</p><ol className="list-decimal ml-5">{a.steps.map((s)=><li key={s}>{s}</li>)}</ol><p><b>Deliverable:</b> {a.deliverable}</p><p><b>Success criteria:</b> {a.success}</p><p><b>Common mistake:</b> {a.commonError}</p><ul className="list-disc ml-5">{a.checklist.map((c)=><li key={c}>{c}</li>)}</ul></div></details>;
      })}</div></section>

      <section className={block}><h3 className="font-black text-xl">Checklist</h3><ul className="list-disc ml-5">{currentModule.checklist.map((c)=><li key={c}>{c}</li>)}</ul></section>
      <section className={`${block} bg-cyan`}><h3 className="font-black text-xl">Badge</h3><p className="font-black">{currentModule.badge.name}</p><p>{currentModule.badge.description}</p><p>Durum: {currentModule.assignments.length>0 && completed===currentModule.assignments.length ? 'Açıldı' : 'Kilitli'}</p></section>
    </div>

    <aside className="space-y-3 xl:sticky xl:top-4 h-fit">
      <section className={`${block} bg-magenta`}><h3 className="font-black text-xl">Assistant Panel</h3><div className="flex flex-wrap gap-1 mt-2">{currentModule.assistantPrompts.map((q)=><button key={q} onClick={()=>setQuestion(q)} className="border-[3px] border-black bg-yellow px-2 py-1 text-xs font-black">{q}</button>)}</div><div className="mt-2 border-[3px] border-dashed border-black p-2 text-sm bg-white">Screenshot upload placeholder</div><textarea value={question} onChange={(e)=>setQuestion(e.target.value)} className="w-full mt-2 border-[3px] border-black p-2" rows={5} placeholder="Sorunu yaz..."/><button onClick={generate} className="mt-2 border-[3px] border-black px-3 py-2 bg-yellow font-black">Mock Answer Generator</button>{answer && <pre className="mt-2 whitespace-pre-wrap border-[3px] border-black p-2 bg-white text-sm">{answer}</pre>}</section>
    </aside>
  </div>;
}
